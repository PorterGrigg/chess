...
public class PawnMovesCalculator implements PieceMovesCalculator {
    ...
    public List<ChessMove> getPossibleMoves(ChessBoard squares, ChessPosition start, ChessPiece piece){
        //for pawn also need to consider the promotion to queen
        ...
        if (nextRow < 8 && nextCol < 9 && nextRow > 1 && nextCol > 0) { //less than 8 and greater than 1 here
            // because if reach 8 then a promotion would need to happen
        ...
        //start movement
        int rowAdvanceStart;
        if ((piece.getTeamColor() == ChessGame.TeamColor.WHITE && presRow == 2) ||
                (piece.getTeamColor() == ChessGame.TeamColor.BLACK && presRow == 7) ){
            //in start position and can move 2 if no pieces in front
        ...
        for(int option: colOptions){
            ...
            if (nextRow < 8 && nextCol < 9 && nextRow > 1 && nextCol > 0) {
                //less than 8 and greater than 1 here because if reach 8 then a promotion would need to happen
                if(squares.getPiece(nextPos) != null &&
